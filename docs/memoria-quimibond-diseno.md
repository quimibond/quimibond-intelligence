...
- [ ] Portar `email-digest` (correo diario) y `extract-pending` / `extract-demand` / `extract-demand-files` (alimentan el digest, usan Claude y Drive). Necesitan `anthropic_api_key` en Vault.
