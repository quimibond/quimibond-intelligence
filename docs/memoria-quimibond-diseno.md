...
| 2026-09-18 | **Fase 3 (parcial) en producción:** ligas determinísticas (`memoria_link_recent`, job `memoria_ligas` */10), `threads.conv_key` (una conversación = un resumen), `memoria_thread_summaries`, `memoria_facts`, grafo `kg_nodes`/`kg_edges` (`kg_refresh_deterministic` nocturno + escritura desde Claude), Edge Function `memory-consolidate` (job `memoria_consolidar` */5, Sonnet), RPCs `memoria_brief` y `memoria_buscar`; pestaña Memoria de Odoo (`qb_memoria` 1.2) muestra la ficha. Ver CLAUDE.md → "Memoria Fase 3" | Migraciones `20260918_memoria_grafo`, `20260918b_memoria_conversaciones`, `20260918c_memoria_cola` aplicadas |
