-- Rename familia "Tela greige (kg)" → "Tela por kg".
--
-- En el catálogo solo-vendibles (Producto Terminado) los productos en kg NO son
-- greige (tela cruda), sino producto terminado que se vende por peso (chiffon,
-- polycotton, telas inspeccionadas). "greige" era un mislabel. Se renombra en
-- los dos lugares donde se clasifica la familia: refresh_product_cost_catalog
-- (explorador de costos) y get_cost_audit_by_family (auditoría de costos).
-- Solo cambia la etiqueta; la lógica de costeo es idéntica.
  ...
  calc AS (
    SELECT p.*,
      ...
      CASE WHEN p.is_import THEN 'importado' WHEN p.is_tejida THEN 'Entretela tejida' WHEN p.is_ent THEN 'Entretela carda'
           WHEN p.uom='kg' THEN 'Tela por kg' WHEN p.category ILIKE '%Tac-%' OR p.category ILIKE '%Acabado%' THEN 'Tela acabado (m)' ELSE 'Otro' END AS familia
