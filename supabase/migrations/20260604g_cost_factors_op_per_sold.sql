-- 2026-06-04g: gastos de OPERACIÓN ÷ lo VENDIDO (no lo producido). Fabricación
-- sigue ÷ inspeccionado. Agrega metros_vendidos_equiv (m + kg×m_per_kg) y
-- factor_op_vendido a get_cost_factors_monthly.
...
RETURNS TABLE(
  mes text, metros_referencia numeric, metros_inspeccion numeric,
  metros_vendidos_equiv numeric, gastos_fabricacion_mxn numeric, gastos_operacion_mxn numeric,
  factor_fab_x_metro numeric, factor_op_x_metro numeric, factor_total_x_metro numeric,
  factor_fab_insp numeric, factor_op_insp numeric, factor_total_insp numeric,
  factor_op_vendido numeric
)
...
metros AS (
  ...
  WHERE (name LIKE 'TL/OP-ACA%' OR name LIKE 'TL/OP-V10%') AND state = 'done' AND qty_produced > 0
  GROUP BY 1
),
insp AS (
...
ventas_equiv AS (
  SELECT to_char(il.invoice_date, 'YYYY-MM') AS mes,
    SUM(CASE WHEN op.uom = 'm' THEN il.quantity
             WHEN op.uom = 'kg' THEN il.quantity * COALESCE(puc.m_per_kg, 0) ELSE 0 END) AS m_equiv
...
SELECT mo.mes, COALESCE(m.m,0)::numeric, COALESCE(i.m,0)::numeric, COALESCE(ve.m_equiv,0)::numeric,
  ...
  CASE WHEN COALESCE(ve.m_equiv,0)>0 THEN ROUND((g.op/ve.m_equiv)::numeric,4) END
FROM months mo
LEFT JOIN metros m ON m.mes=mo.mes
LEFT JOIN insp i ON i.mes=mo.mes
LEFT JOIN ventas_equiv ve ON ve.mes=mo.mes
