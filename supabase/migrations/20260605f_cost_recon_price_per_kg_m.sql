-- 2026-06-05f: reporte de costo reconstruido — precio de venta en $/m y $/kg.
--
-- get_full_cost_reconstruction expone dos columnas nuevas para que el frontend
-- pueda mostrar el precio de venta en ambas unidades:
--   kg_per_unit = peso kg/m (de product_kg_per_unit) — para tela en metros.
--   m_per_kg    = metros por kg (de product_uom_conversion) — para tela en kg.
-- El precio nativo = ventas/cantidad; el otro se convierte: tela en m → $/kg =
-- $/m ÷ kg_per_unit; tela en kg → $/m = $/kg ÷ m_per_kg.
