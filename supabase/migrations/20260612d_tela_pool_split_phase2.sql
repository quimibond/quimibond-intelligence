-- 2026-06-12d: FASE 2 — split quirúrgico del pool de tela (saca entretelas).
--
-- Tras 20260612c las entretelas costean con su propio factor, PERO su costo
-- (MOD centro ENTRETELAS + renta Lote 10) seguía en el pool de tela y sus metros/
-- kg en el denominador -> doble conteo + las telas sub-costeadas (la entretela,
-- ligera y barata de hacer, diluía el denominador en kg).
--
-- Fix: en get_cost_factors_monthly, los factores que costean tela
-- (factor_fab_peso_kg_smooth, factor_fab_largo_m_smooth) ahora usan:
--   numerador = pool fab − (MOD ENTRETELAS + renta contractual Lote 10)
--   denominador kg = kg inspeccionados − kg de entretela
--   denominador m  = metros inspeccionados − metros de entretela
-- Guard: solo se resta desde 2026-01 (cuando entretelas pasó a ser centro con
-- renta propia; rent_lot_assignment.effective_from = 2026-01-01). El suavizado
-- 12m absorbe la volatilidad de la renta del GL.
--
-- Impacto medido: factor de fabricación de TELA sube ~+4% a +24% (prom ~+11%) —
-- es correcto: se quita el "subsidio" que la entretela le daba a la tela. Op y
-- columnas legacy (factor_fab_kg, factor_fab_insp, etc.) NO cambian. Las
-- entretelas usan su factor propio (no se ven afectadas por este cambio).
...
insp AS (
  SELECT to_char(csm.date,'YYYY-MM') AS mes, SUM(csm.quantity) AS m,
         ...
         SUM(csm.quantity * kpu.kg_per_unit) FILTER (WHERE (cp.internal_ref !~ ' I$' OR cp.internal_ref IS NULL) AND op.category ILIKE '%Entretela%' AND op.category NOT ILIKE '%Importaci%') AS kg_ent
...
ent AS (  -- costo de entretela a restar del pool de tela (solo 2026+)
  SELECT mo.mes,
...
base AS (
  SELECT mo.mes,
    ...
    COALESCE(i.m_ent,0)::numeric AS insp_m_ent, COALESCE(i.kg_ent,0)::numeric AS kg_insp_ent,
...
SELECT b.mes, b.met, b.insp_m, b.vend_equiv, b.fab, b.op,
  ...
  ROUND(
    ...
    + (SUM(CASE WHEN b.op>0 AND b.kg_vend>0 THEN b.op ELSE 0 END) OVER w)
    / NULLIF(SUM(CASE WHEN b.op>0 AND b.kg_vend>0 THEN b.kg_vend ELSE 0 END) OVER w, 0), 4),
  -- componente PESO ($/kg) = ws × fab_TELA/kg_TELA suavizado (entretelas fuera)
  ROUND( (SELECT ws FROM cfg) *
    (SUM(CASE WHEN (b.fab-b.fab_ent)>0 AND (b.kg_insp-b.kg_insp_ent)>0 THEN (b.fab-b.fab_ent) ELSE 0 END) OVER w)
    / NULLIF(SUM(CASE WHEN (b.fab-b.fab_ent)>0 AND (b.kg_insp-b.kg_insp_ent)>0 THEN (b.kg_insp-b.kg_insp_ent) ELSE 0 END) OVER w, 0), 4),
  -- componente LARGO ($/m) = (1-ws) × fab_TELA/metro_TELA suavizado (entretelas fuera)
