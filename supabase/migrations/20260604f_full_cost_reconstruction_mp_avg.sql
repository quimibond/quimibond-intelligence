-- 2026-06-04f: get_full_cost_reconstruction devuelve también la MP a costo
-- PROMEDIO (avg_cost, vía get_bom_raw_material_cost_per_unit) junto a la MP a
-- último costo de compra, para comparar (drift de precios de materia prima).
-- (Cuerpo idéntico a la función aplicada en producción; ver columnas
--  costo_primo_avg_unit_mxn / costo_primo_avg_total_mxn al final del TABLE.)
DROP FUNCTION IF EXISTS public.get_full_cost_reconstruction(text);
CREATE FUNCTION public.get_full_cost_reconstruction(p_period text)
RETURNS TABLE(
...
base AS (
  SELECT COALESCE(r.pid, q.pid) AS pid, cp.internal_ref AS product_ref, cp.display_name AS product_name,
    ...
    CASE WHEN op.uom = 'm' THEN 1
         WHEN op.uom = 'kg' THEN COALESCE(puc.m_per_kg, 0)
         ELSE 0 END AS conv,
    CASE
...
calc AS (
  SELECT b.*, COALESCE(f.ff,0) * b.conv AS fab_unit, COALESCE(f.fo,0) * b.conv AS op_unit
...
SELECT
  ...
  c.mp_source,
  ROUND(c.primo_avg_unit, 4),
  ROUND(c.qty_sold * c.primo_avg_unit, 2)
FROM calc c
WHERE c.qty_sold <> 0 OR c.revenue <> 0;
$function$;
