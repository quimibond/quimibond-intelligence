-- 2026-06-17c: el catalogo de costos por producto solo incluye PRODUCTO TERMINADO
-- (vendibles). Antes el filtro (categoria ILIKE '%Tac-%'/'%Entretela%') colaba
-- materia prima (hilo, quimico, resina, fibra) y productos en proceso (greige,
-- tejido circular base) = componentes. Ahora: category ILIKE 'Producto Terminado%'.
-- Pasa de 2,931 a 1,354 productos (sin materia prima ni en proceso). El cuerpo
-- completo se aplico via apply_migration (identico al anterior salvo el WHERE del
-- CTE prod). Alimenta /contabilidad/costos-producto.
