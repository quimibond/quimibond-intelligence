-- ============================================================================
-- Migration 20260416: Fix burn_rate_weekly formula + self-sufficient sources
--
...
--
-- Además: la versión anterior de cashflow_liquidity_metrics dependía de
-- cashflow_ar_predicted y cashflow_ap_predicted que NO están en producción
-- (solo existen en migrations 20260415_projected_cash_flow_v2 y v3 que
-- fueron aplicadas parcialmente). Esta versión computa overdue_ar /
-- overdue_ap directo de odoo_invoices (fuente de verdad) y no depende
-- de vistas intermedias frágiles.
-- ============================================================================
...
WITH
  ...
  -- AR vencida: directo de odoo_invoices en MXN canónico
  overdue_ar AS (
    SELECT COALESCE(SUM(COALESCE(amount_residual_mxn, amount_residual)), 0)::numeric AS ar_overdue
    FROM odoo_invoices
  ...
  overdue_ap AS (
    SELECT COALESCE(SUM(COALESCE(amount_residual_mxn, amount_residual)), 0)::numeric AS ap_overdue
    FROM odoo_invoices
...
COMMENT ON VIEW cashflow_liquidity_metrics IS
  'Métricas de liquidez para get_cashflow_recommendations. burn_rate_weekly = payroll_quincenal*7/15 + opex_weekly + tax_weekly. ap_overdue_coverage_ratio en PORCENTAJE (0-100+). overdue_ar/ap directos de odoo_invoices (MXN canónico).';
