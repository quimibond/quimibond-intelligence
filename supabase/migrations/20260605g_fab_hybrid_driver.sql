-- 2026-06-05g: fabricación HÍBRIDA por driver (peso + largo).
--
-- El CEO observó (correcto) que repartir 100% de la fabricación por PESO
-- sobre-castiga a las telas pesadas: una tela pesada tiene MENOS metros por kg,
-- y los procesos que corren por metro (acabado/rama a velocidad de línea,
-- inspección) no tardan más con una tela pesada. Drivers confirmados con el CEO:
--   TEJIDO + TINTORERIA -> PESO (kg): hilo, químicos, agua, calor escalan con kg.
--   ACABADO + ENTRETELAS -> LARGO (metros): corren por velocidad de línea.
-- Split ~47% peso / 53% largo (de la distribución por centro de costo).
--
-- factor_fab_peso_kg_smooth = ws * (fab/kg inspeccionado, suavizado 12m)
-- factor_fab_largo_m_smooth = (1-ws) * (fab/metro inspeccionado, suavizado 12m)
-- fab del producto: tela en m -> kg_per_m*peso_kg + largo_m;
--                   tela en kg -> peso_kg + m_per_kg*largo_m.
-- ws = costing_config.fab_weight_share (editable, default 0.47).

CREATE TABLE IF NOT EXISTS public.costing_config (
...
INSERT INTO public.costing_config (key, value, notes) VALUES
  ('fab_weight_share', 0.47,
   'Fraccion de la fabricacion por PESO (TEJIDO+TINTORERIA, ~47%). El resto por LARGO/metro (ACABADO+ENTRETELAS). Editable.')
...
SELECT b.mes, b.met, b.insp_m, b.vend_equiv, b.fab, b.op,
  ...
  ROUND( (1 - (SELECT ws FROM cfg)) *
    (SUM(CASE WHEN b.fab>0 AND b.insp_m>0 THEN b.fab ELSE 0 END) OVER w)
    / NULLIF(SUM(CASE WHEN b.fab>0 AND b.insp_m>0 THEN b.insp_m ELSE 0 END) OVER w, 0), 4)
FROM base b
