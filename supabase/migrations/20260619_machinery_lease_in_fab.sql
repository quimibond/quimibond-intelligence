-- Incluir el arrendamiento de MAQUINARIA (701.11) en el pool de fabricación.
--
-- Confirmado con el CEO (2026-06-19): el leaseback financiero 701.11.0001
-- ($4.54M YTD Ene–May, ~$909k/mes) es el arrendamiento de la maquinaria
-- PRODUCTIVA — NO es lo mismo que la renta de la nave/oficinas (504.01.0008 +
-- 603.45). Son activos distintos. Como la maquinaria se usa para producir, su
-- costo es overhead fijo de fabricación (análogo a la depreciación de
-- maquinaria propia) y debe absorberse al producto. Antes vivía 100% debajo
-- del EBIT y no tocaba ningún costo de producto.
--
-- Cambio: se agrega `account_code LIKE '701.11%'` al filtro `fab` de la CTE
-- `gastos`. Como es "general toda la producción", fluye por el split híbrido
-- peso/largo del pool de tela (las entretelas usan su propio factor, que NO
-- toca este pool — la maquinaria arrendada es del tren de tela). Efecto: pool
-- de fabricación Ene–May $27.76M → $32.30M (+16%); factor de fab por unidad
-- sube ~16%.
--
-- NOTA: esto es costeo gerencial (absorción). El P&L financiero (pnl.ts)
-- mantiene 701.11 debajo del EBIT como financiamiento — no se toca.
  ...
  SELECT to_char(csm.date,'YYYY-MM') AS mes, SUM(csm.quantity) AS m,
         ...
         SUM(csm.quantity * kpu.kg_per_unit) FILTER (
            WHERE (cp.internal_ref !~ ' I$' OR cp.internal_ref IS NULL)
              ...
              AND (op.name ILIKE '%no tejid%' OR NOT (op.name ILIKE '%tejido circular%' OR op.name ILIKE '%tejida%' OR op.category ILIKE '%Puntos%'))
         ) AS kg_ent_carda
