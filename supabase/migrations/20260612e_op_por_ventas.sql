-- 2026-06-12e: operación (admin/ventas) por % de VENTAS, no por kg.
--
-- Antes: op_unit = factor_op_kg × kg_per_unit. Cargaba la operación por PESO,
-- penalizando a las telas pesadas por metro (una tela pesada no da más trabajo
-- de admin/ventas que una ligera). No respetaba la eficiencia que sí logramos
-- en fabricación con el reparto híbrido.
--
-- Ahora: op_unit = op_pct × precio_venta_unit, donde op_pct = gastos 6xx ÷
-- ventas, suavizado 12m (guard op_pool>0 para excluir el cierre de diciembre).
-- Admin+ventas escalan con cuánto vendes en pesos, no con kilos. Es el
-- tratamiento estándar de SG&A y no castiga a la pesada por su peso.
--
-- Efecto: las pesadas/baratas pagan algo menos de op (X140 $6.10->$5.87,
-- WC090 $4.04->$3.25), las ligeras un poco más. gastos_op_total = op_pct ×
-- revenue por producto. op_pct ~17.9% en 2026. Cache v21->v22.
...
op_pct AS (  -- gastos 6xx ÷ ventas, suavizado 12m (excluye meses de cierre con op<=0)
