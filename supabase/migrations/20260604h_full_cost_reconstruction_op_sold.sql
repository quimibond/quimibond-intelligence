-- 2026-06-04h: get_full_cost_reconstruction usa factor_op_vendido (operación ÷
-- vendido) y excluye subproductos (SALDO/DESPERDICIO) del reporte. Fabricación
-- sigue ÷ inspeccionado. Mantiene MP último + promedio y conversión kg.
...
WITH factors AS (
  SELECT factor_fab_insp AS ff, factor_op_vendido AS fo
...
base AS (
  SELECT COALESCE(r.pid, q.pid) AS pid, cp.internal_ref AS product_ref, cp.display_name AS product_name,
    ...
    CASE WHEN op.uom = 'm' THEN 1 WHEN op.uom = 'kg' THEN COALESCE(puc.m_per_kg, 0) ELSE 0 END AS conv,
