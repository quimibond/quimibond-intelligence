...
-- ============================================================
-- Orchestrator: run all internal invariants and return summary
-- ============================================================
...
BEGIN
  ...
  -- fx_sanity STUBBED — schema lacks exchange_rate. View is always empty;
  -- we force severity='ok' so inventory count remains 16 regardless.
  SELECT COUNT(*) INTO v_count FROM v_audit_invoice_lines_fx_sanity;
  PERFORM _audit_register_invariant(p_run_id, p_date_from, p_date_to,
    'invoice_lines.fx_sanity', 'invoice_lines', v_count, 'ok');
...
COMMENT ON FUNCTION run_internal_audits IS
  'Ejecuta 16 invariantes SQL internos (A-O, D stubbed) y escribe filas a audit_runs con run_id provisto. '
  'Spec: docs/superpowers/specs/2026-04-19-sync-audit-design.md';
