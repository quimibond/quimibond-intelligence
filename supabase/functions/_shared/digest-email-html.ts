/**
 * Render HTML del "Resumen de correo" diario (email-digest). Copia de
 * src/lib/alerts/digest-email-html.ts para Edge Functions (sin CTA al frontend).
 *
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:100%;">
  ...
  <!-- CTA (solo si hay sistema al que enlazar) -->
  <tr><td align="center" style="background:${C.card};border-left:1px solid ${C.line};border-right:1px solid ${C.line};border-radius:0 0 12px 12px;border-bottom:1px solid ${C.line};padding:${systemUrl ? "26px 32px 30px" : "8px 32px 16px"};">
    ${systemUrl ? `<a href="${esc(systemUrl)}" style="display:inline-block;background:${C.accent};color:#ffffff;font-family:${FONT};font-size:14px;font-weight:700;text-decoration:none;padding:12px 28px;border-radius:8px;">Ver en el sistema&nbsp;→</a>` : ""}
  </td></tr>
