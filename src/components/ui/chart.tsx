...
// shadcn/ui Chart primitive — canonical wrapper around recharts.
// Adaptado a recharts 3.x (TooltipContentProps / LegendPayload).
// Usar siempre <ChartContainer config={config}> y <ChartTooltipContent>
// en vez de crear wrappers propios sobre recharts.
...
// Payload item as delivered by recharts at runtime (shape is dynamic —
// different chart types populate different keys). Typed loosely on purpose.
type TooltipItem = {
...
// Helper to extract config item from payload
function getPayloadConfigFromPayload(
  ...
  if (key in (payload as Record<string, unknown>)) {
    ...
    if (typeof candidate === "string") configLabelKey = candidate;
  } else if (payloadPayload && key in payloadPayload) {
    const candidate = payloadPayload[key];
    if (typeof candidate === "string") configLabelKey = candidate;
  }
