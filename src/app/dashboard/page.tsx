...
async function fetchDashboardFallback(): Promise<DirectorDashboard> {
  ...
  const [
    ...
    supabase.from("action_items").select("id, description, contact_name, contact_company, assignee_email, assignee_name, due_date, priority, reason, action_type").eq("state", "pending").lt("due_date", today).order("due_date", { ascending: true }).limit(10),
