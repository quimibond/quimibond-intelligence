...
export default async function FixDetailPage({
  ...
  // Try the indexed cache first; fall back to direct fetch
  ...
  return (
    <PageLayout>
      ...
      {insight.insight_type === "odoo_partner_no_canonical" && (
        <PartnerNoCanonicalDetail
          odooPartnerId={ev.odoo_partner_id as number}
        />
      )}
...
async function SatInvoiceDriftDetail({
  ...
  const cols: DataTableColumn<SatInvoiceDriftDetailRow>[] = [
    {
      ...
      cell: (r) => (
        <span className="font-mono text-[11px]">
          {r.uuid_sat ? r.uuid_sat.slice(0, 8) + "…" : "—"}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (r) => (
        <Badge
          variant={severityVariant[r.severity as OdooFixSeverity] ?? "secondary"}
          className="text-[10px]"
        >
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={AlertCircle}
        title="Sin issues subyacentes"
        description="El insight reporta count > 0 pero no se encontraron rows. Refresca la página."
      />
    );
  }
...
// ─────────────────────────────────────────────────────────────────
// B) Duplicate partner RFC detail
// ─────────────────────────────────────────────────────────────────
...
async function DuplicatePartnerRfcDetail({ rfc }: { rfc: string }) {
  ...
  const cols: DataTableColumn<DuplicatePartnerRfcDetailRow>[] = [
    ...
    },
    {
      key: "country",
      header: "País",
      cell: (r) => <span className="text-xs">{r.country ?? "—"}</span>,
      hideOnMobile: true,
    },
    {
      key: "type",
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={AlertCircle}
        title="Sin partners con este RFC"
        description="Probablemente ya fueron mergeados. Refresca el detector."
      />
    );
  }
...
// ─────────────────────────────────────────────────────────────────
// C) Partner no canonical detail
// ─────────────────────────────────────────────────────────────────

async function PartnerNoCanonicalDetail({
  odooPartnerId,
}: {
    ...
    },
    {
      ...
      cell: (r) => (
        <span className="font-semibold tabular-nums">
          {fmtMxn(r.amount_mxn_sat)}
        </span>
      ),
    },
    {
      key: "partner_name",
      header: "Partner name (raw)",
      alwaysVisible: true,
      cell: (r) => <span className="text-sm">{r.partner_name ?? "—"}</span>,
    },
    {
      key: "sat_uuid_complemento",
      header: "UUID Complemento",
      cell: (r) => (
        <span className="font-mono text-[11px]">
          {r.sat_uuid_complemento
            ? r.sat_uuid_complemento.slice(0, 8) + "…"
            : "—"}
        </span>
      ),
      hideOnMobile: true,
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={AlertCircle}
        title="Sin pagos para este partner"
        description="Probablemente ya se canonicalizó."
      />
    );
  }
  ...
  return (
    <Card data-table-export-root>
      <CardHeader className="flex flex-row items-center justify-between gap-4 space-y-0">
        <CardTitle className="text-base">
          {rows.length} pagos · partner Odoo #{odooPartnerId}
        </CardTitle>
...
// ─────────────────────────────────────────────────────────────────
// D) Foreign tax-id detail
// ─────────────────────────────────────────────────────────────────

async function ForeignTaxIdDetail({
  odooPartnerId,
}: {
  odooPartnerId: number;
}) {
  ...
  if (!data) {
    return (
      <EmptyState
        ...
        description={`No existe companies.odoo_partner_id = ${odooPartnerId}.`}
      />
    );
  }
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-4 space-y-0">
        <CardTitle className="text-base">
          Partner #{data.odoo_partner_id} · {data.name ?? "(sin nombre)"}
        </CardTitle>
        <Button asChild variant="outline" size="sm" className="h-9 gap-1.5">
          <a
            href={`https://quimibond.odoo.com/web#id=${data.odoo_partner_id}&model=res.partner&view_type=form`}
            target="_blank"
      ...
      <CardContent>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
          <div>
            ...
            <dd className="font-mono text-base font-semibold text-warning">
              {data.rfc ?? "—"}
            </dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="flex gap-1">
              {data.is_customer && <Badge variant="info">Cliente</Badge>}
              {data.is_supplier && <Badge variant="secondary">Proveedor</Badge>}
          ...
          <div className="sm:col-span-2">
            ...
            <dd className="font-mono text-base font-semibold text-success">
              XEXX010101000
            </dd>
            <p className="text-xs text-muted-foreground mt-1">
              RFC genérico para partners extranjeros. Guardar el tax-id real
              ({data.rfc}) en el campo Notes o un custom field.
            </p>
          </div>
        </dl>
      </CardContent>
...
async function ContactsDuplicatesDetail() {
  ...
  const cols: DataTableColumn<ContactDuplicateGroup>[] = [
    {
      key: "name",
      header: "Nombre canónico",
      alwaysVisible: true,
      cell: (g) => <span className="font-medium">{g.canonical_name}</span>,
    ...
    },
    {
      ...
      align: "right",
      cell: (g) => (
        <span className="font-semibold tabular-nums">{g.dup_count}</span>
      ),
    },
    {
      key: "ids",
    ...
    },
    {
      ...
      cell: (g) => (
        <span className="text-xs text-muted-foreground line-clamp-2">
          {g.members.map((m) => m.email ?? "(null)").join(" · ")}
        </span>
      ),
      className: "min-w-[260px]",
    },
  ...
  if (groups.length === 0) {
    return (
      <EmptyState
        icon={AlertCircle}
        title="Sin grupos duplicados"
        description="Todos los contactos tienen canonical_name único."
      />
    );
  }
...
async function ProductsDuplicatesDetail() {
  ...
  const cols: DataTableColumn<ProductDuplicateGroup>[] = [
    ...
    },
    {
      ...
      align: "right",
      cell: (g) => {
        const total = g.members.reduce(
          (acc, m) => acc + (m.stock_qty ?? 0),
          0
        );
        return (
          <span className="font-semibold tabular-nums">
  ...
  if (groups.length === 0) {
    return (
      <EmptyState
        icon={AlertCircle}
        title="Sin grupos duplicados"
        description="Todos los productos tienen canonical_name único."
      />
    );
  }
  ...
  return (
    <Card data-table-export-root>
      <CardHeader className="flex flex-row items-center justify-between gap-4 space-y-0">
        <CardTitle className="text-base">
          {groups.length} grupos · ⚠ son variantes legítimas, NO duplicates reales
        </CardTitle>
